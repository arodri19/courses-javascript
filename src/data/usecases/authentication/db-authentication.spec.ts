...
describe('DbAuthentication UseCase', () => {
  ...
  test('Should throw if LoadAccountByEmailRepository throws', async () => {
    ...
    const promise = sut.auth(makeFakeAutthentication())
    await expect(promise).rejects.toThrow()
  })
  test('Should return null if LoadAccountByEmailRepository returns null', async () => {
    const { sut, loadAccountByEmailRepositoryStub } = makeSut()
    jest.spyOn(loadAccountByEmailRepositoryStub, 'load').mockReturnValueOnce(null)
    const accessToken = await sut.auth(makeFakeAutthentication())
  ...
  test('Should call TokenGerator with correct id', async () => {
    const { sut, tokenGeneratorStub } = makeSut()
    const generateSpy = jest.spyOn(tokenGeneratorStub, 'generate')
    await sut.auth(makeFakeAutthentication())
    expect(generateSpy).toHaveBeenCalledWith('any_id')
  })
  test('Should throw if TokenGerator throws', async () => {
    const { sut, tokenGeneratorStub } = makeSut()
    jest.spyOn(tokenGeneratorStub, 'generate').mockReturnValueOnce(new Promise((resolve, reject) => reject(new Error())))
    const promise = sut.auth(makeFakeAutthentication())
    await expect(promise).rejects.toThrow()
  })
  test('Should return correct access token', async () => {
    const { sut } = makeSut()
    const accessToken = await sut.auth(makeFakeAutthentication())
